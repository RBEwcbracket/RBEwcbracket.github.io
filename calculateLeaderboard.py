...
def calculate_leaderboard():
    ...
    for user_entry in raw_predictions[1:]:
        ...
        # Track actual points earned, not just counts
        stats = {"winnerPoints": 0, "exactPoints": 0, "closePoints": 0}
        ...
        for m_idx, match in enumerate(matches):
            ...
            if match_id in results_map:
                ...
                if predW == res['winner']:
                    ...
                    # 3. Close Score (Goal Difference)
                    else:
                        pred_diff = abs(predA - predB)
                        act_diff = abs(res['scoreA'] - res['scoreB'])
                        if pred_diff == act_diff:
                            match_points_earned += points_config['close']
                            stats["closePoints"] += points_config['close']
    ...
    print("Leaderboard generated successfully with updated point breakdowns and visual ranking!")
